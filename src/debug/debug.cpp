...
std::unordered_map<std::string, std::unordered_map<std::string, uint64_t> (*)()> storeList{
    {"Texture", Texture::getList},   {"SpriteMap", SpriteMap::getList}, {"Shader", Shader::getList},
    {"Pipeline", Pipeline::getList}, {"Sampler", Sampler::getList},
};
...
int debugConsole()
{
    ...
    while(!GameEng::GameState::gameClosing())
    {
        if(poll(&pol, POLLIN, 50))
        {
            ...
            if(!GameEng::GameState::gameClosing())
                std::cout << "\r\ndebug: " << std::flush;
        }
    }
    return 0;
