...
namespace Packager
{
PackageManager::PackageManager(const std::string& file) : packageFile(file), fileList({})
{
    ...
    try
    {
        data.resize(dataStart - sizeof(uint64_t));
    }
    catch(std::bad_alloc const& err)
    {
        throw GameEngineException(GEError::INVALID_FILE_FORMAT, "Unable to allocate " +
                                                                    std::to_string(dataStart - sizeof(uint64_t)) +
                                                                    " bytes for " + file);
    }
        ...
        if(file.name == path)
        {
            ...
            do
            {
                ...
                if(ferror(package) != 0)
                {
                    ...
                    fclose(package);
                    throw GameEng::GameEngineException(GameEng::GEError::FILE_IO,
                                                       "Could not read from package: " + packageFile);
                }
                if(strm.avail_in == 0)
                {
                    break;
...
const std::string& PackageManager::getPackageName() const
{
    return packageFile;
}
...
void createPackage(const std::string& directory, const std::string& file)
{
    ...
    while(readSize == CHUNK)
    {
        ...
        if(writeCount != readSize)
        {
            fclose(openFile);
            fclose(compData);
            throw GameEngineException(GEError::FILE_IO,
                                      "Could not write entire compressed data to package file: " + file);
        }
    }
    fclose(openFile);
    fclose(compData);
...
std::vector<std::string> getDirContents(const std::string& directory)
{
    ...
    catch(std::filesystem::filesystem_error const& err)
    {
        throw GameEngineException(GEError::FILE_NOT_FOUND,
                                  "Could not get file list of " + directory + ": " + err.code().message());
    }
}
