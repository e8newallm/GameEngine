...
class World
{
  public:
    ...
    /**
     * \brief Registers a function to run when the world is updated (useful for things such as checking keyboard/mouse
     * inputs).
     *
    ...
    /**
     * \brief Runs one physics update tick if it is time for a new update tick (As long as the physics isn't paused and
     ...
     */
    [[nodiscard]] bool physicsRunning() const
    {
        return phyRunning;
    };
     ...
     */
    void setPhyInterpolation(double newPhyInter)
    {
        phyInterPercent = newPhyInter;
    };
     ...
     */
    [[nodiscard]] double getPhyInterpolation() const
    {
        return phyInterPercent;
    };
