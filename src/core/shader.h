...
{
/**
 * \brief Class for wrapping SDL_GPUShader* for use with the DataStore in the class. Also has functions for creating
 * SDL_GPUShader objects.
 *
 */
class Shader : public DataStore<Shader, Shader>
{
  public:
     ...
     */
    Shader(SDL_GPUShader* shader, SDL_GPUDevice* newGPU) : shader(shader), gpu(newGPU) {};
     ...
     */
    [[nodiscard]] static std::vector<uint8_t> buildShader(const std::string& filename,
    ...
    [[nodiscard]] static std::shared_ptr<Shader> LoadShaderFromArray(
        SDL_GPUDevice* gpu, const std::string& shaderFilename, const std::vector<uint8_t>& code, Uint32 samplerCount,
     ...
     * \return std::shared_ptr<Shader> The resulting shader saved in the GPU.
     */
    [[nodiscard]] static std::shared_ptr<Shader> LoadShaderFromFile(SDL_GPUDevice* gpu,
     ...
     * \return std::shared_ptr<Shader> The resulting shader saved in the GPU.
     */
    [[nodiscard]] static std::shared_ptr<Shader> createShader(SDL_GPUDevice* gpu, const std::string& filename,
